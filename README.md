...
## Recommended Specs
Fallout 3 is pretty old, so you do not need very powerful hardware to run the vanilla game at 60 FPS with medium-high settings. The modlist is pretty lightweight, so it shouldn't have higher system requirements than the vanilla game. It's recommended that you install the game onto an SSD to improve load times and decrease stuttering.
...
- [evilneko](https://www.nexusmods.com/profile/evilneko) for the [solution](https://forums.nexusmods.com/topic/129406-fallout-3-will-not-run-at-all/#findComment-1509734) to why the game crashes on startup for some people.
